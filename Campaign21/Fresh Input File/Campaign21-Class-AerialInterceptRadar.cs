...
public class AIRadarTarget
{
    ...
    private void calcDistancePip()
    {
        ...
        if (this.targetDistance_m >  2*AIRadarRadius_m || this.targetDistance_m > playerPos.z * 2.5 || this.targetDistance_m < 133)
        {
            inRange = false;
    ...
    private void setOutOfRangePips()
    {
        ...
        if (this.targetDistance_m > 2.0 * AIRadarRadius_m)  //So, we're giving them a break and giving the direction to target (only) starting @ 2x the radar distance
    ...
    public double displayPips()
    {
        ...
        disp += " " + (XDist_m / 1000.0).ToString("F1") + " " + (YDist_m / 1000.0).ToString("F1") + " " + (ZDist_m / 1000.0).ToString("F1") + " " + targetRelativeAngle_deg.ToString("F0");
...
public class AIRadarMission : AMission
{
    ...
    public void handleAIRadarRequest(Player player)
    {
        ...
        try
        {
            ...
            {
                GamePlay.gpLogServer(new Player[] { player }, "AIRadar: No aircraft nearby to track, sorry. {0}", new object[] { aircraftList.Count });
                return;
            }
            else
            {
