...
public class AIRadarTarget : IDisposable
{
...
public class AIRadarTargetArray : IDisposable
{
...
public class AIRadarAuthenticArray : AIRadarTargetArray, IDisposable
{
    ...
    private double AIRadarMagnification_max = 8; //We can zoom in 1, 2, 4, 8 etc X magnification
    ...
    {       
        ...
        double distanceLimit_m = (playerPos.z * 2); //approximating that the radar couldn't see out further than the plane was tall
        int XLimitPos = Convert.ToInt32(Math.Round(distanceLimit_m / AIRadarRadius_m * ((double)GridWidth - 1) / 2.0 + ((double)GridWidth - 1) / 2.0)) + 1;
        int XLimitNeg = Convert.ToInt32(Math.Round(-distanceLimit_m / AIRadarRadius_m * ((double)GridWidth - 1) / 2.0 + ((double)GridWidth - 1) / 2.0)) - 1;
        int YLimit = Convert.ToInt32(Math.Round((distanceLimit_m * 1.5) / AIRadarRadius_m * ((double)GridHeight))) + 1;
    ...
    public override void placeItemInGrid(Point3d p)
    {
        //int XPos, YPos, Zpos = 0;       //so 0,0,0 is our "ERROR" or too large value, as we can't place an item at 0,0,0.
        //if (Math.Abs(XDist_m)<=AIRadarRadius_m)
        int XPos = Convert.ToInt32(Math.Round(p.y * AIRadarMagnification / AIRadarRadius_m * ((double)GridWidth - 1) / 2.0 + (double)GridWidth / 2.0));
        int YPos = Convert.ToInt32(Math.Round(p.x * AIRadarMagnification / AIRadarRadius_m * ((double)GridHeight)));

        int ZPos = Convert.ToInt32(Math.Round(p.z * AIRadarMagnification / AIRadarRadius_m * ((double)GridWidth - 1) / 2.0 + (double)GridWidth / 2.0));
...
public class AIRadarMission : AMission
{
    ...
    public AIRadarMission(Mission msn)
    {
        ...
        mainmission = msn;
        //asvradarmission = mainmission.asvradarmission;
    ...
    void Mission_EventChat(Player player, string msg)
    {
        ...
        if (msg.StartsWith("<as") && !msg.StartsWith("<asv"))
        ...
        {
            GamePlay.gpLogServer(new Player[] { player }, ">>>>AIRBORNE INTERCEPT RADAR OPERATION DETAILS (part 6/6)", null);
        ...
        else if (msg.StartsWith("<ahelp4") || msg.StartsWith("<ah4"))
        {
            ...
            GamePlay.gpLogServer(new Player[] { player }, "Airborne Intercept Radar displays do not have a definite distance scale.", null);
            ...
            GamePlay.gpLogServer(new Player[] { player }, "Look behind capabilities are very limited or nonexistent.  Look behind radar came later in the war.", null);
        ...
        else if (msg.StartsWith("<ahelp") || msg.StartsWith("<ah"))
        {
            ...
            GamePlay.gpLogServer(new Player[] { player }, "Tab-4-4-5 is same as <aa.  Repeat the command to cycle through magnifications & turn the radar off.", null);
    ...
    public string handleAIplusASVTab4StatusString(Player player)
    {
        try
        {
            string ai_mode = "(off)";

            if (PlayerCurrentAIRadarAuthenticArray != null && PlayerCurrentAIRadarAuthenticArray.ContainsKey(player))
            {
                //airadarmission.AIRadarAuthenticArray AIRTA = PlayerCurrentAIRadarAuthenticArray[player];
                if (PlayerCurrentAIRadarAuthenticArray[player].turnedOn)
                {
                    double res = PlayerCurrentAIRadarAuthenticArray[player].AIRadarMagnification;
                    ai_mode = "(ON-mag" + res.ToString("n0") + "X)";
                }
            }
            else if (mainmission.asvradarmission != null && mainmission.asvradarmission.PlayerCurrentAIRadarAuthenticArray != null && mainmission.asvradarmission.PlayerCurrentAIRadarAuthenticArray.ContainsKey(player))
            {
                //airadarmission.AIRadarAuthenticArray AIRTA = PlayerCurrentAIRadarAuthenticArray[player];
                if (mainmission.asvradarmission.PlayerCurrentAIRadarAuthenticArray[player].turnedOn)
                {
                    double res = mainmission.asvradarmission.PlayerCurrentAIRadarAuthenticArray[player].AIRadarMagnification;
                    ai_mode = "(ASV ON-mag" + res.ToString("n0") + "X)";
                }
            }
            return ai_mode;
        } catch (Exception ex) { Console.WriteLine("handleAIplusASVTab4StatusString ERROR " + ex.ToString()); return "(off)"; }
    }

    //status 0 = all radar off, AI & ASV
    //1-4 = 4 modes of AI
    //5-6 = 2 modes of ASV
    public int handleAIplusASVTab4Request(Player player, double delay_s = 7)
    {
        try { 
        int status = 0;
        ...
        if (PlayerCurrentAIRadarAuthenticArray != null && PlayerCurrentAIRadarAuthenticArray.ContainsKey(player))
        {
            ...
            {
                double res = PlayerCurrentAIRadarAuthenticArray[player].AIRadarMagnification;
                status = Convert.ToInt32(1+ Math.Log(res, 2));
            }
        }
        else if (mainmission.asvradarmission != null && mainmission.asvradarmission.PlayerCurrentAIRadarAuthenticArray != null && mainmission.asvradarmission.PlayerCurrentAIRadarAuthenticArray.ContainsKey(player))
        {
            ...
            {
                double res = mainmission.asvradarmission.PlayerCurrentAIRadarAuthenticArray[player].AIRadarMagnification;
                status = Convert.ToInt32(5 + Math.Log(res, 2));
            }
        }
        Console.WriteLine("Airborn Radar (Authentic) & ASV Radar command received - current status: {0}", status);

        if (status < 4)
        {
            handleAIRadarAuthenticArrayRequest(player, delay_s); //turn on AIR or bump up magnification

            //turn off ASV radar just in case
            if (mainmission.asvradarmission != null && mainmission.asvradarmission.PlayerCurrentAIRadarAuthenticArray != null && mainmission.asvradarmission.PlayerCurrentAIRadarAuthenticArray.ContainsKey(player))
            {
                ASVRadarAuthenticArray AIRTA = mainmission.asvradarmission.PlayerCurrentAIRadarAuthenticArray[player];
                AIRTA.turnOff();
                AIRTA.Dispose();
                AIRTA.delete();
            }
        ...
        {
            //turn off Ai (only necessary if status == 4 but won't hurt anything if status == 5
            if (PlayerCurrentAIRadarAuthenticArray != null && PlayerCurrentAIRadarAuthenticArray.ContainsKey(player))
            {
            ...
            //turn on ASV and/or bump on magnification
            if (mainmission.asvradarmission != null && mainmission.asvradarmission.PlayerCurrentAIRadarAuthenticArray != null) mainmission.asvradarmission.handleAIRadarAuthenticArrayRequest(player, delay_s); //turn on ASVR or bump up magnification
        }
        else
        {//turn everything off
            status = -1; //because we're ++ing it below, we'll end up with 0
            //AIRadarAuthentic
            if (PlayerCurrentAIRadarAuthenticArray!= null && PlayerCurrentAIRadarAuthenticArray.ContainsKey(player))
            {
                AIRadarAuthenticArray AIRTA = PlayerCurrentAIRadarAuthenticArray[player];
        ...
        catch (Exception ex) { Console.WriteLine("handleAIplusASVTab4Request: ERROR " + ex.ToString()); return 0; }
    }

    public string handleAIRadarAuthenticTab4StatusString(Player player)
    {
        string ai_mode = "(off)";
    ...
    public void handleAIRadarAuthenticArrayRequest(Player player, double delay_s = 7)
    {
        ...
        if (!isPlayerAllowedAIRadar(player))
        {
            GamePlay.gpLogServer(new Player[] { player }, "AIRadar: Radar not allowed for your aircraft (must be 2 or more seats; can't be carrying cargo or ferrying).", new object[] { });
