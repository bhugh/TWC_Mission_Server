...
public class KnickebeinTarget
{
    ...
    KnickebeinMission mission;
   
    
    public KnickebeinTarget(Player player, Point3d initialPoint, Point3d targetPoint, KnickebeinMission mission)
    {
        this.player = player;
    ...
    public KnickebeinTarget(Player player, Point3d targetPoint, KnickebeinMission mission)
    {
        this.mission = mission;
        chatOrHud = ChatOrHud.Hud;
        this.player = player;        
    ...
    public KnickebeinTarget(Player player, double angle_deg, double dist, KnickebeinMission mission)
    {
        this.mission = mission;
        chatOrHud = ChatOrHud.Hud;
        this.player = player;
    ...
    public void turnOn()
    {
        if (mission.mainmission.objectiverepairmission.orm_PlayersOnRepairMissionExpiration.ContainsKey(player))
        {
            this.turnedOn = false;
            return;
        };
    ...
    */
    ...
    public double displayPips(Player player=null)
    {
        ...
        if (mission.mainmission != null && mission.mainmission.objectiverepairmission !=null && mission.mainmission.objectiverepairmission.orm_PlayersOnRepairMissionExpiration.ContainsKey(player)) return 0; //deny knickebein to ppl flying repair missions; their a/c arent equipped with KB
        Tuple<double, string> distT = distancePip(player);
        double dist = distT.Item1;
        string disp = directionPip(player) + " " + distT.Item2;

        //KB gets shakey when flying very low over land (but not water...)
        double Z_AltitudeAGL_m = 10000;
        if (actor as AiAircraft != null) Z_AltitudeAGL_m = (actor as AiAircraft).getParameter(part.ParameterTypes.Z_AltitudeAGL, 0);
        if (Z_AltitudeAGL_m < 75)
        {
            maddox.game.LandTypes landType = mission.GamePlay.gpLandType(player.Place().Pos().x, player.Place().Pos().y);
            if (landType != maddox.game.LandTypes.WATER && Z_AltitudeAGL_m < mission.ran.Next(45,75) )
            {
                disp = "????";
            }

            if (!mission.recvdKBMalfunctionMsg.Contains(player))
            {
                mission.GamePlay.gpLogServer(new Player[] { player }, ">>>Knickebein: Knickebein reception is very poor when you are flying very low ...", new object[] { });
                mission.recvdKBMalfunctionMsg.Add(player);
            }
        }
        
        if (mission.isKnickebeinDisabled(player)) disp = "[[[Knickebein HQ Destroyed - no signal]]]";
        
        if (mission.GamePlay != null)
        {
            //if (this.chatOrHud == ChatOrHud.Chat) mission.GamePlay.gpLogServer(new Player[] { player }, disp, new object[] { }); 
            //else 
            mission.GamePlay.gpHUDLogCenter(new Player[] { player }, disp);
        }
        return dist;
    ...
    private void display_recurs()
    {
            ...
            if (mission.mainmission.objectiverepairmission.orm_PlayersOnRepairMissionExpiration.ContainsKey(this.player))
            {
                Console.WriteLine ( "Knickebein: Aircraft used to shuttle Repair Cargo Loads are not outfitted with Knickebein, sorry!", null);
                this.turnOff();                
                return;
            }
...
public class Knickebeinholder
{
    ...
    public Dictionary<Player, Point3d> knickebeinAnchorPoint { set; get; }
    public Dictionary<Player, AiActor> knickebeinAnchorPlace { set; get; }

    public KnickebeinMission mission;

    public Knickebeinholder(KnickebeinMission mission)
    {
        this.mission = mission;
        ...
        knickebeinAnchorPoint = new Dictionary<Player, Point3d>();
        knickebeinAnchorPlace = new Dictionary<Player, AiActor>();
        Console.WriteLine("Knickebeinholder inited!");

    }

    public void setAnchorPoint(Player player) //so the anchor point is set when the player first sets any KB point, and that is where the signals all always "emanate" from. It is changed only if the player enters a new plane/vehicle & then the new "emanation point" is from that point
    {
        if (player == null || player.Place() == null) return;

        //Never established an anchor place OR in a new plane/actor
        if (!knickebeinAnchorPoint.ContainsKey(player) || !knickebeinAnchorPlace.ContainsKey(player) || knickebeinAnchorPlace[player] != player.Place())
        {
    ...
    public Point3d getAnchorPoint(Player player) //so the anchor point is set when the player first sets any KB point, and that is where the signals all always "emanate" from. It is changed only if the player enters a new plane/vehicle & then the new "emanation point" is from that point
    {
        Point3d nullPoint3d = new Point3d(-1, -1, -1);

        if (player == null || player.Place() == null) return nullPoint3d;

        //Never established an anchor place OR in a new plane/actor
        if (!knickebeinAnchorPoint.ContainsKey(player)) setAnchorPoint(player);
        if (!knickebeinAnchorPoint.ContainsKey(player)) return nullPoint3d;
        return knickebeinAnchorPoint[player];
    ...
    public bool KniStart(Player player, bool contin = false, bool next = false, bool prev = false, int startWay_onebased = 0)
    {
        ...
        //deny use to repair delivery planes
        if (mission.mainmission.objectiverepairmission.orm_PlayersOnRepairMissionExpiration.ContainsKey(player))
        {
            if (mission.GamePlay != null) mission.GamePlay.gpLogServer(new Player[] { player }, "Knickebein: Aircraft used to shuttle Repair Cargo Loads are not outfitted with Knickebein, sorry!", new object[] { });
            KniStop(player, false);
            if (knickebeins.ContainsKey(player)) knickebeins.Remove(player);
            return true;
        } else
        {
            Console.WriteLine("Knickebein: Player NOT using RepairMission");
        ...
        //if (!contin && !((next || prev) && currWay == currWaySave)) knickebeins[player] = new KnickebeinTarget(player, getAnchorPoint(player), currPoint, mission);

        knickebeins[player].turnOn();

        if (mission.GamePlay != null && !mission.isKnickebeinDisabled(player)) mission.GamePlay.gpLogServer(new Player[] { player }, "Knickebein: Beam heading is {0:F0} degrees magnetic", new object[] { Math.Round(knickebeins[player].targetBearingAngle_magnetic_deg) }); //note the gplogserver IGNORES any formatting requests such as N0 or F0 . . . .
        return true;
    ...
    public bool KniOnStartOrNext(Player player)
    {
        ...
        if (mission.mainmission.objectiverepairmission.orm_PlayersOnRepairMissionExpiration.ContainsKey(player))
        {
            //this.turnedOn = false;
            Console.WriteLine("Knickebein: Aircraft used to shuttle Repair Cargo Loads are not outfitted with Knickebein, sorry!", null);
            if (knickebeins.ContainsKey(player)) knickebeins[player].turnOff();
            if (knickebeins.ContainsKey(player)) knickebeins.Remove(player);
            return false;
        }
        else
        ...
        if (mission.GamePlay != null)
        {
            if (!mission.isKnickebeinDisabled(player)) mission.GamePlay.gpLogServer(new Player[] { player }, "Knickebein: Starting Knickebein waypoint #{0} to {1} ({2:N0},{3:N0})", new object[] { currWay + 1, sector, Math.Round(currPoint.x), Math.Round(currPoint.y) });
            else mission.GamePlay.gpLogServer(new Player[] { player }, "Knickebein: Loading Knickebein waypoint #{0} to {1} ({2:N0},{3:N0}) [[[Knickebein HQ currently disabled; KB points only for communicating with <Cover aircraft]]]", new object[] { currWay + 1, sector, Math.Round(currPoint.x), Math.Round(currPoint.y) });
        }
        ...
        if (currWay != currWaySave)
        {
            ...
            //knickebeins[player] = new KnickebeinTarget(player, getAnchorPoint(player), currPoint, mission);
        }
        /*
        ...
        */
        Console.WriteLine("KOSON6");
        knickebeins[player].turnOn();

        //if (mission.GamePlay != null) mission.GamePlay.gpLogServer(new Player[] { player }, "Knickebein: Beam bearing {0:F0} degrees magnetic", new object[] { Math.Round(knickebeins[player].targetBearingAngle_magnetic_deg) }); //note the gplogserver IGNORES any formatting requests such as N0 or F0 . . . .

        if (mission.GamePlay != null && !mission.isKnickebeinDisabled(player)) mission.GamePlay.gpLogServer(new Player[] { player }, "Knickebein: Beam heading {0:F0} degrees magnetic", new object[] { Math.Round(knickebeins[player].targetBearingAngle_magnetic_deg) }); //note the gplogserver IGNORES any formatting requests such as N0 or F0 . . . .

        Console.WriteLine("KOSON7");
        return true;
    ...
    public void KniInfo(Player player)
    {
        ...
        if (mission.isKnickebeinDisabled(player))
        {

            if (mission.GamePlay != null)
            {
                mission.GamePlay.gpLogServer(new Player[] { player }, "The Knickebein system is down because Knickebein HQ was destroyed! Current Waypoint: #{0}  {5} ({6:N0}/{7:N0})", new object[] { currWay + 1, Math.Round(knickebeins[player].targetBearingAngle_magnetic_deg), initialSector, Math.Round(knickebeins[player].initialPoint.x), Math.Round(knickebeins[player].initialPoint.y), targetSector, Math.Round(knickebeins[player].targetPoint.x), Math.Round(knickebeins[player].targetPoint.y) });

                mission.GamePlay.gpLogServer(new Player[] { player }, "You can still use this point to communicate a target to your <Cover aircraft.  However, your KB guidance display is inoperative.", null);
            }
    ...
    */
    public string KniList(Player player, bool display = true, bool html = false )
    {
        ...
        if (mission.isKnickebeinDisabled(player)) msg = string.Format("Your currently active Knickebein Waypoint: #{0}. [[[Knickebein HQ destroyed - KB display inoperative.]]]", new object[] { currWay + 1, bearing_deg });
        retmsg += msg + newline;
        mission.GamePlay.gpLogServer(new Player[] { player }, msg, null);
...
public class KnickebeinMission : AMission, IKnickebeinMission
{
    ...
    public HashSet<Player> recvdKBMalfunctionMsg = new HashSet<Player>();

    public KnickebeinMission(Mission msn)
    {
    ...
    //Set the knickebein anchor point whenever a player enters an aircraft
    ...
    */
    ...
    public override void OnBattleStoped()
    {
        ...
        knickeb = new Knickebeinholder(this);
        knickeb.KniTest();
        Console.WriteLine("-knickebein exiting onbattlestoped");
    }
    ...
    void Mission_EventChat(Player from, string msg)
    {
        ...
        */
        ...
        else if (msg.StartsWith("<ka")) //<kadd
        {
            try
            {
                ...
                string newmsg = msg.Replace("<kadd", "").Replace("<ka", "").Replace(",", " ").Replace("[", " ").Replace("]", " ").Replace("(", " ").Replace(")", " ").Replace("  ", " ").Replace("  ", " ").Replace("  ", " ").Replace("|", ":").Replace(";", ":").Replace("!", ":").Replace("*", ":").Trim(); // remove the comma, parentheses etc

                List<Point3d> points = new List<Point3d>();
                bool anyEntries = false;

                string[] phrases = newmsg.Split(':');
                foreach (string phrase in phrases)
                {
                    if (phrase.Length > 1) anyEntries = true;
                ...
                else
                {
                    ...
                    GamePlay.gpLogServer(new Player[] { player }, "Or several at once: <kadd 142381 150321 : 213433 184333 : 284332 312324 : 175444 08223 : BA14.3.2", null);
                    GamePlay.gpLogServer(new Player[] { player }, "(may use : ; | ! * to delimit entries)", null);
                    GamePlay.gpLogServer(new Player[] { player }, "Then to use: Tab-4-4-4-4 menu OR <kstart <knext <kprev <koff <kon <kclear <khelp", null);
                }

                if (isKnickebeinDisabled(player)) {

                    Timeout(7, () => { GamePlay.gpLogServer(new Player[] { player }, ">>>IMPORTANT: Your Knickebein Central Headquarters has been destroyed!  Your Knickebein wayfinding system is inoperative!", null); });
                    Timeout(7.02, () => { GamePlay.gpLogServer(new Player[] { player }, ">>>You can still use Knickebein points to communicate targets to your <Cover aircraft.", null); });
                    Timeout(7.04, () => { GamePlay.gpLogServer(new Player[] { player }, ">>>But your KB display will be inoperative until KB Central HQ is repaired!", null); });
        ...
        */
        ...
        else if (msg.StartsWith("<khelp2") || msg.StartsWith("<kh2"))
        {
            ...
            GamePlay.gpLogServer(new Player[] { player }, "Or several at once: <kadd 142381 150321 : 213433 184333 : 284332 312324 : 175444 08223 : BA14.3.2", null);
            GamePlay.gpLogServer(new Player[] { player }, "(may use : ; | ! * to delimit entries)", null);
            GamePlay.gpLogServer(new Player[] { player }, "Main Knickebein commands are on the Tab-4-4-4-4 menu OR Chat Commands <kstart <knext <kprev <koff <kon <kclear", null);
        ...
        else if (msg.StartsWith("<khelp") || msg.StartsWith("<kh"))
        {
            ...
            GamePlay.gpLogServer(new Player[] { player }, ">>>>KNICKEBEIN BASIC OPERATION", null);
            ...
            GamePlay.gpLogServer(new Player[] { player }, "Knickebein operation depends on the status of your Knickebein/Radar Beam Navigation HQ!!!!", null);
            GamePlay.gpLogServer(new Player[] { player }, "If the HQ is destroyed, your Knickebein system will become inoperative until HQ is repaired.", null);
            GamePlay.gpLogServer(new Player[] { player }, "However - you can still use KB to communicate target locations to your <Cover aircraft. But your own navigation is disabled.", null);
    ...
    public List<bool> isKnickebeinDisabled_store = new List<bool>() { false, false, false };
    private List<DateTime> lastKBHQCheck = new List<DateTime>() { DateTime.UtcNow.AddDays(-1), DateTime.UtcNow.AddDays(-1), DateTime.UtcNow.AddDays(-1) };

    //public bool isKnickebeinDisabled(Player player)
    //{ return false; }
        
        public bool isKnickebeinDisabled(Player player)
    {
        if (player == null) return false;
        return isKnickebeinDisabled(player.Army());
    }
    public bool isKnickebeinDisabled(int army)
    {
        try
        {            
            if (army < 1 || army > 2) return false; //I guess we'll say they're not involved in the war, all is good
                                                    //if the army has at least one knickebeinHQ in working order/not destroyed
                                                    //then their knickebein system is up
            if (DateTime.UtcNow.Subtract(lastKBHQCheck[army]).TotalMinutes < 20) return isKnickebeinDisabled_store[army];
            
            if (mainmission == null) return false;
			int kb_count = 0;
			int good_kb_count = 0;
            ...
            foreach (Mission.MissionObjective mo in mainmission.MissionObjectivesList.Values){
                ...
                if (mo.MOObjectiveType != Mission.MO_ObjectiveType.KnickebeinHQ || !mo.IsEnabled) continue;
			...
			//no KBHQs found at all OR they are all working
			if (kb_count <=0 || good_kb_count == kb_count ) {
				   isKnickebeinDisabled_store[army] = false;
                   return false;
			}
			
			double kb_pct = (double)good_kb_count / (double)kb_count;
			
			
			//so knocking out half the KBHQs will knock out the KB entirely
			//before that, any KBHQs knocked out will lead to ever-greater chance of outages
			double knockout = ran.NextDouble()/2.0 + 0.5;			
			...
			//KB is disabled for this 20 min period
            isKnickebeinDisabled_store[army] = true;
            return true;

        } catch (Exception ex) { Console.WriteLine("Knickebein isKnickebeinDisabled ERROR: " + ex.ToString()); return false; }
    }
...
public static class KniCalcs
{
    ...
    public static string correctedSectorNameDoubleKeypad(KnickebeinMission msn, Point3d p)
    {
    ...
    public static string correctedSectorNameKeypad(KnickebeinMission msn, Point3d p)
    {
    ...
    public static string correctedSectorName(KnickebeinMission msn, Point3d p)
    {
    ...
    public static int giantkeypad(Point3d p)
    {
        ...
        double sizex = 360000;
        double sizey = 310000; //Tobruk is 360000
    ...
    public static void loadSmokeOrFire(maddox.game.IGamePlay GamePlay, KnickebeinMission mission, double x, double y, double z, string type, double duration_s = 300, string path = "")
    {
        /* Samples: 
