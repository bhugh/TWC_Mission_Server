...
public class SkinCheckMission : AMission
{
    ...
    HashSet<string> LargeSkins = new HashSet<string> () { };

    public void DeleteLargeSkinFiles(bool deleteFile = false)
    {
        try
        {
            //if (deleteFile) Console.WriteLine("Skincheck: Checking for large skin files . . . ");
            List<string> files = GetImageFilenamesFromDirectory(CLOD_PATH + "cache/", deleteFile);
            foreach (string fileName in files)
            {
                try
                {
                    ...
                    //if (deleteFile) Console.WriteLine("Checking skin file " + fileName);
                    if (fileSize > 500000)
                    {
                        if (!LargeSkins.Contains(fileName))
                        {
    ...
    //
    public List<string> GetImageFilenamesFromDirectory(string dirPath, bool deleteFile = false)
    {
        List<string> list = new List<string>();
        if (deleteFile) Console.WriteLine("Skincheck: Looking for skin files in: " + dirPath);
        ...
        if (deleteFile) Console.WriteLine("Skincheck: Num skin files found: " + list.Count);
        return list;
    }
