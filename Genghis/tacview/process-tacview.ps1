# Create a unique name for your script's lock
$mutexName = "Global\TWC_Process_Tacview_Lock"
...
if (-not $createdNew) {
    Write-Warning "Another instance of this script is already running. Exiting immediately."
    exit
}
...
try {
	...
	# Find and remove matching files older than 2 days in main folder
	Get-ChildItem -Path $ScriptDir -File  | 
		Where-Object { 
			($_.Name -like "Tacview-*-COD-Genghis-Class-TacViewImplMission.zip.acmi" -or $_.Name -like "Tacview-*-COD-Genghis-Class-TacViewImplMission.txt.acmi") -and 
			$_.LastWriteTime -lt $CutoffDate 
		} | 
		Remove-Item -Force
		
	$CutoffDate = (Get-Date).AddDays(-7)	
	# Find and remove matching files older than 7 days in main folder + all subfolders
	Get-ChildItem -Path "$ScriptDir\uploaded" -File | 
		Where-Object { 
			($_.Name -like "*.zip.acmi*" -or $_.Name -like "*.txt.acmi*") -and 
			$_.LastWriteTime -lt $CutoffDate 
		} | 
		Remove-Item -Force
		
	Get-ChildItem -Path "$ScriptDir\processed" -File | 
		Where-Object { 
			($_.Name -like "*.zip.acmi*" -or $_.Name -like "*.txt.acmi*") -and 
			$_.LastWriteTime -lt $CutoffDate 
		} | 
	...
	Get-ChildItem -Path "$ScriptDir\originals" -File | 
		Where-Object { 
			($_.Name -like "*.zip.acmi*" -or $_.Name -like "*.txt.acmi*") -and 
			$_.LastWriteTime -lt $CutoffDate 
		} | 
    ...
    # Always release and dispose of the mutex when finished
    if ($mutex) {
        $mutex.ReleaseMutex()
        $mutex.Dispose()
    }
