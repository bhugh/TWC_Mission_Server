...
public class MissionNeg10 : TWCTobrukCampaignMissionObjectives
{
    public Mission msn;
    public Mission.MissionObjectives mmo;
    public maddox.game.IGamePlay gp;
    public double score; //-1000000.111 is a "magic number" meaning no subclass
    ...
    private double? leastScore = -15; //Every subclass MUST include this value, which is the least campaign score value for which it will be loaded (>=)
    private double? mostScore = -5;   //Every subclass MUST include this value, which is the greatest campaign score value for which it will be loaded (<)
    ...
    public MissionNeg10(maddox.game.IGamePlay g, Mission m, Mission.MissionObjectives mo, double sc)
    ...
    */
    ...
    //public Dictionary<ArmiesE, GeneralStaffLocationObject> GeneralStaffLocations = new Dictionary<ArmiesE, GeneralStaffLocationObject>();
    ...
    public override void OverrideCampaignTurnMapRequirements()
    {
        ...
        mo.MO_PercentPrimaryTargetsRequired = new Dictionary<ArmiesE, double>() {
            {ArmiesE.Red, 80 },
            {ArmiesE.Blue, 80 }
        };
        ...
        //at all
        mo.MO_PointsRequiredToTurnMap = new Dictionary<ArmiesE, double>() {
            {ArmiesE.Red, 120 },
            {ArmiesE.Blue, 120 }
        };


        mo.MO_BRBumrushInfo = new Dictionary<ArmiesE, M.MO_BRBumrushInfoType>() {
        ...
        if (current_subclass.HasValue)
        {
            try
            {
                TWCTobrukCampaignMissionObjectives(current_subclass.Value).OverrideCampaignTurnMapRequirements();
...
RTarget54R TGroundDestroyed 100 17036 77666 50
        ...
        */
        ...
        if (current_subclass.HasValue)
        {
            try
            {
                TWCTobrukCampaignMissionObjectives(current_subclass.Value).RadarPositionTriggersSetup(addNewOnly);
            }
            catch (Exception ex) { Console.WriteLine("TWCTobrukCampaignMissionObjectives ERROR: " + ex.ToString()); }
        }
        */
    ...
    public override void MissionObjectiveTriggersSetup(bool addNewOnly = false)
    {
        ...
        if (current_subclass.HasValue)
        {
            try
            {
                current_subclass.HasValue.MissionObjectiveTriggersSetup(addNewOnly);
            }
            catch (Exception ex) { Console.WriteLine("TWCTobrukCampaignMissionObjectives ERROR: " + ex.ToString()); }
        }
...
added Rouen Flak 
    ...
    public void BumRushCampaignValuesSetup()
    {
